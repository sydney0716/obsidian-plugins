...
This plugin for [Obsidian](https://obsidian.md) allows you to automatically generate and maintain index notes for your folders. It keeps track of files, ensuring your folder notes are always up-to-date.

# Features

- **Automatic Indexing**: Generates an index file containing links to all files within a directory.
- **Abstract Extraction**: Automatically extracts and displays the text under `## 0. Abstract` from each note.
...
- **Customizable Templates**: Define exactly how your index note should look using placeholders like `{folderName}` and `{files}`.
...
### Abstract Extraction
To display an abstract for a note in the index, add the following section to your note:

```markdown
## 0. Abstract
This is the summary of the note.
It will appear in the index file.
```
...
- **Template**: Customize the content of the index file.
  ...
  - `{files}`: A list of links to files in the folder (with abstracts).
...
이 [Obsidian](https://obsidian.md) 플러그인은 폴더에 대한 인덱스 노트를 자동으로 생성하고 관리해줍니다. 파일들을 추적하여 폴더 노트가 항상 최신 상태를 유지하도록 돕습니다.

# 주요 기능

- **자동 인덱싱 (Automatic Indexing)**: 폴더 내의 모든 파일 링크가 포함된 인덱스 파일을 생성합니다.
- **요약 추출 (Abstract Extraction)**: 각 노트의 `## 0. Abstract` 섹션 내용을 자동으로 추출하여 인덱스에 표시합니다.
...
- **커스텀 템플릿 (Customizable Templates)**: `{folderName}`, `{files}`와 같은 변수를 사용하여 인덱스 노트의 형식을 원하는 대로 지정할 수 있습니다.
...
### 요약 추출 (Abstract Extraction)
인덱스에 노트의 요약을 표시하려면, 노트에 다음 섹션을 추가하세요:

```markdown
## 0. Abstract
이곳에 노트의 요약 내용을 작성하세요.
작성된 내용은 인덱스 파일에 함께 표시됩니다.
```
